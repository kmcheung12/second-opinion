---
name: second-opinion
description: Use when the user wants to ask another model or harness for a second opinion on the current problem. Triggers on phrases like "ask kimi", "ask opencode", "ask claude opus", "second opinion", "ask another model".
---
...
If no model hint is given, omit `-m` and let the harness use its default.
